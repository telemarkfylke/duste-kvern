...
const handleSystem = async (system, correspondingSystemInOverview, report, mongoCollection) => {
  ...
  let getDataFunction
  try {
    getDataFunction = require(`../systems/${system.id}/get-data.js`)
  } catch (error) {
...
const handleDustReport = async (report) => {
  ...
  await collection.updateOne({ _id: new ObjectId(report._id) }, { $set: { finishedTimestamp: new Date().toISOString(), systems: systemsOverview } })
