...
const invoke = (scriptName, caller, args) => {
  // Setup full path to script
  const scriptPath = `${PS1_SCRIPTS_PATH}/${scriptName}`

  // Validate scriptPath
  if (!existsSync(scriptPath)) {
    throw new Error(`'${scriptName}' does not exist`)
  }

  if (!scriptPath.toLowerCase().endsWith('.ps1')) {
    throw new Error(`'${scriptName}' is not a PowerShell script`)
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    logger('info', ['invoke-ps-script', caller, 'executing command', cmdPwsh])
    ...
    const proc = exec(cmd, { cwd: dirname(scriptPath), maxBuffer: Number.parseInt(MAX_BUFFER) }, (error, stdout, stderr) => {
      if (stderr !== '') {
        ...
        logger('error', ['invoke-ps-script', caller, 'exec stderr', proc.pid, message])
        // eslint-disable-next-line
        return reject({ message, stack })
      }
      if (error !== null) {
        const { message, stack } = getError(scriptPath, stderr)
        logger('error', ['invoke-ps-script', caller, 'exec error', proc.pid, message])
        // eslint-disable-next-line
        return reject({ message, stack })
      }

      logger('info', ['invoke-ps-script', caller, 'exec finished', proc.pid])
      try {
